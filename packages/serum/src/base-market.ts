...
import { divideBnToNumber, getFilteredProgramAccounts, getPriceFromKey, throwIfNull } from './utils';
...
export class BaseMarket {
  protected _decoded: any;
  protected _baseSplTokenDecimals: number;
  protected _quoteSplTokenDecimals: number;
  protected _skipPreflight: boolean;
  protected _commitment: Commitment;
  protected _programId: PublicKey;
  protected _openOrdersAccountsCache: {
    [publickKey: string]: { accounts: OpenOrders[]; ts: number };
  };

  protected _feeDiscountKeysCache: {
    [publicKey: string]: {
      accounts: Array<{
        balance: number;
        mint: PublicKey;
  ...
  static async findAccountsByMints(
    ...
    const filters = [
      ...
      {
        memcmp: {
          offset: BaseMarket.getLayout(programId).offsetOf('quoteMint'),
...
export class Orderbook {
    ...
    constructor(market: BaseMarket, accountFlags, slab: Slab) {
    ...
    static decode(market: BaseMarket, buffer: Buffer) {
